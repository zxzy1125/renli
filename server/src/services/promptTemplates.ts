...
export const PARSE_POSITION_PROMPT = {
  system: BASE_SYSTEM,
  user: `从职位原文提取结构化字段。原文可能来自 .txt/.pdf/.docx/.xlsx/.csv/图片，可能含格式残渣，需智能识别。如附带图片，必须识别图片内所有可见文字（标题/表格/JD/薪资/地点等）并整合到对应字段，图片与文本冲突时优先采信图片数字/术语。

【职位原文】
{raw_text}

输出 JSON（所有字段必填，原文未提填 null）：
{
  "title": "职位名（不明确则按职责推断）",
  "clientCompany": "客户公司名",
  "department": "部门",
  "salaryMin": "薪资下限（纯数字字符串，单位 K）",
  "salaryMax": "薪资上限（纯数字字符串）",
  "salaryUnit": "K/W/元/月/元/天（未提默认 K）",
  "experience": "经验要求",
  "education": "学历要求",
  "location": "工作地点",
  "headcount": "招聘人数（纯数字字符串）",
  "jobType": "枚举：fulltime/parttime/intern/outsourcing",
  "workMode": "枚举：onsite/remote/hybrid",
  "priority": "枚举：high/medium/low（按薪资/紧急/HC判断，默认 medium）",
  "responsibilities": "岗位职责（逐条保留原文，Markdown 有序列表，禁止合并）",
  "requirements": "任职要求（逐条保留原文，Markdown 有序列表，禁止合并）",
  "bonus": "加分项（Markdown 列表，无则空字符串）",
  "highlights": ["职位亮点 3-5 条（每条≤30字，具体禁止套话）"],
  "keywords": ["关键词 5-10 个"],
  "salaryDetails": { "baseSalary": "底薪", "commission": "提成方式与比例", "performanceBonus": "绩效奖金规则", "annualBonus": "年终奖/十三薪等", "totalRange": "综合薪资范围", "otherCompensation": "股票/期权/项目奖金等其他收入" },
  "trainingPeriod": { "duration": "培训期时长", "salary": "培训期薪资", "content": "培训内容" },
  "probation": { "duration": "试用期时长", "salary": "试用期薪资/比例", "conditions": "转正条件", "postSalary": "转正后薪资结构" },
  "performanceMetrics": { "kpi": "业绩指标/考核标准", "evaluationCycle": "考核周期（月度/季度/年度）", "consequences": "未达标后果/奖惩" },
  "benefits": { "socialInsurance": "五险一金", "paidLeave": "带薪假期", "allowances": "各类补贴", "insurance": "商业保险", "meals": "餐饮/住宿", "activities": "团建/旅游/体检/节日福利", "other": "其他福利" },
  "workLifeBalance": { "overtime": "枚举：rare/occasional/frequent/heavy", "schedule": "上下班时间", "weekdays": "休息安排", "flexibility": "弹性工作说明" },
  "teamInfo": { "size": "团队规模", "reportTo": "汇报对象", "subordinates": "下属人数", "structure": "团队架构" },
  "growthPath": { "promotion": "晋升通道", "training": "培训/带教", "learning": "学习资源" },
  "positionContext": { "hiringReason": "枚举：new/replacement/expansion/unknown（未提填 unknown）", "urgency": "枚举：high/medium/low", "note": "其他补充" },
  "interviewProcess": { "rounds": "面试轮数", "format": "面试形式", "duration": "面试/入职周期" },
  "companyInsights": { "scale": "公司规模", "industry": "行业", "fundingStage": "融资阶段", "highlights": "公司亮点" },
  "jobSeekerVerdict": "2-3 句话从求职者角度客观评价优劣势，风险点（加班/薪资不明/提成不确定）必须如实指出",
  "confidence": 0.85,
  "uncertainFields": ["把握不大的字段（<0.85 时必填）"],
  "rawTextSummary": "1-2 句话总结原文核心"
}

铁律：
1. 逐字提取，禁止编造/臆测/美化，原文未提必须填 null，宁可空也不可瞎编
2. responsibilities/requirements 必须逐条保留原文（宁多勿少）；salaryDetails/benefits/trainingPeriod/probation/performanceMetrics/workLifeBalance/teamInfo/growthPath/interviewProcess/companyInsights 等文本字段尽量用原文原话或精简版，禁止扩写
3. 枚举字段（jobType/workMode/priority/overtime/hiringReason/urgency）必须从给定值中选，禁止自创；salaryMin/salaryMax 必须纯数字字符串，原文只给一个数时上下限填同值
4. Excel 多 sheet 必须整合所有 sheet 不漏；图片内文字必须纳入解析
5. 多职位识别：若原文含多个独立职位（不同标题/部门/拼接），输出 {"positions":[{...},{...}]} 数组，每个对象字段结构同上；单职位直接输出对象，不要包 positions 数组
6. confidence：信息齐全=0.9+；部分推断=0.7-0.85；大量缺失=<0.7。输出必须合法 JSON，不要 markdown 代码块`,
};
