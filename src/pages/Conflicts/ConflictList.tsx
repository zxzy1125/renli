...
export default function ConflictList() {
  ...
  return (
    <div className="px-6 py-6 max-w-6xl mx-auto">
      <div className="mb-4">
        <h1 className="font-serif text-2xl font-bold text-forest-800">撞单管理</h1>
...
function ConflictCard({
  ...
  return (
    <div className="card p-5">
      ...
      {/* 备注 */}
      {conflict.note && (
        <div className="mt-3 px-3 py-2 rounded-lg bg-cream-50 text-sm text-forest-600">
          <span className="text-forest-400">备注：</span>
          {conflict.note}
        </div>
      )}
      ...
      {/* 操作按钮 */}
      {showActions && (
        <div className="mt-4 flex flex-wrap items-center gap-2 pt-3 border-t border-forest-50">
          <span className="text-sm text-forest-500 mr-2">处理方式：</span>
          <button
            type="button"
          ...
          <button
            ...
            className="text-xs px-3 py-1.5 rounded text-risk-600 hover:bg-risk-50 border border-risk-100"
          >
            标记误报
          </button>
        </div>
...
function ResumeSide({
  ...
  const sideColor = side === 'A' ? 'bg-forest-50 text-forest-700' : 'bg-ochre-50 text-ochre-700';
  return (
    <div className="border border-forest-100 rounded-lg p-3 flex flex-col gap-2">
      ...
      {resume ? (
        ...
        <div className="text-xs text-forest-400 italic">简历信息加载失败</div>
      )}
