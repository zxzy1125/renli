  ...
  return (
    <div className="px-6 py-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="font-serif text-2xl font-bold text-forest-800">对话辅助</h1>
  ...
  return (
    <tr className="border-t border-forest-50 hover:bg-cream-50/50">
      ...
      <td className="px-4 py-3">
        <Link
          to={`/chat/${session.id}`}
          className="font-medium text-forest-800 hover:text-forest-600 flex items-center gap-1.5"
        >
        ...
        {session.candidate_name && session.title && session.candidate_name !== session.title && (
          <div className="text-xs text-forest-400 mt-0.5">求职者：{session.candidate_name}</div>
        )}
      </td>
      {/* 职位 */}
      <td className="px-4 py-3">
        ...
        {positionSalary && (
          <div className="text-xs text-forest-400 mt-0.5">薪资 {positionSalary}</div>
        )}
      </td>
      {/* 简历 */}
      <td className="px-4 py-3">
        {resumeName ? (
          <span className="text-forest-700">{resumeName}</span>
        ) : (
          <span className="text-xs text-forest-400">未绑定</span>
        )}
      </td>
      {/* 状态 */}
      <td className="px-4 py-3">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-forest-700 mb-1">
            求职者名 <span className="text-forest-400 text-xs">（可选，未绑定简历时建议填写）</span>
          </label>
          <input
            type="text"
            className="input"
        ...
        <div>
          ...
          <div className="border border-forest-100 rounded-lg max-h-48 overflow-y-auto bg-cream-50/30">
            {posLoading ? (
              <div className="p-3 text-sm text-forest-400 text-center">
                <Loader2 className="w-4 h-4 animate-spin inline mr-1" />
                加载中...
              </div>
            ) : positions.length === 0 ? (
              <div className="p-3 text-sm text-forest-400 text-center">暂无可选职位</div>
            ) : (
              positions.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm border-b border-forest-50 last:border-b-0 transition-colors ${
                      selected
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {selected && <Check className="w-3.5 h-3.5 text-forest-600" />}
                    </div>
        ...
        <div>
          <label className="block text-sm font-medium text-forest-700 mb-1">
            绑定简历 <span className="text-forest-400 text-xs">（可选，绑定后 AI 分析更精准）</span>
          </label>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          <div className="border border-forest-100 rounded-lg max-h-48 overflow-y-auto bg-cream-50/30">
            ...
            <button
              type="button"
              onClick={() => setResumeId('')}
              className={`w-full text-left px-3 py-2 text-sm border-b border-forest-50 transition-colors ${
                resumeId === ''
                  ? 'bg-forest-100 text-forest-800'
                  : 'hover:bg-cream-100 text-forest-700'
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="text-forest-500">不绑定简历</span>
                {resumeId === '' && <Check className="w-3.5 h-3.5 text-forest-600" />}
              </div>
            </button>
            {resLoading ? (
              <div className="p-3 text-sm text-forest-400 text-center">
              ...
              <div className="p-3 text-sm text-forest-400 text-center">暂无可选简历</div>
            ) : (
              resumes.map((r) => {
        ...
        {/* 提示 */}
        <div className="px-3 py-2 rounded-lg bg-cream-50 border border-forest-100 text-xs text-forest-500">
